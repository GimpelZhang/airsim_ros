...
class PosePath3D(object):
    ...
    def get_speed_acc(self):
        speeds=[]
        acc=[]
        return speeds, acc  # no idea yet

    def get_angular_speed(self):
        ang_speeds=[]
        return ang_speeds  # no idea yet
...
class PoseTrajectory3D(PosePath3D, object):
    ...
    def get_speed_acc(self):
        """
        :return: speed and linear acceleration of the trajectory
        """
        speeds, acc = super(PoseTrajectory3D, self).get_speed_acc()
        speeds = [
            calc_speed(self.positions_xyz[i], self.positions_xyz[i + 1],
                       self.timestamps[i], self.timestamps[i + 1])
            for i in range(len(self.positions_xyz) - 1)
        ...
        acc = [
            calc_linear_acc(speeds[i], speeds[i + 1],
                       self.timestamps[i+1], self.timestamps[i + 2])
            for i in range(len(self.positions_xyz) - 2)
        ]
        return speeds, acc
    
    def get_angular_speed(self):
        """
        :return: angular speed of the trajectory
        """
        ang_speeds = super(PoseTrajectory3D,self).get_angular_speed()
        ang_speeds = [
            calc_angular_speed(self.poses_se3[i],self.poses_se3[i+1],self.timestamps[i],self.timestamps[i+1])
            for i in range(len(self.poses_se3)-1)
        ]
        return ang_speeds
...
def calc_linear_acc(speed_1, speed_2, t_1, t_2):
    """
    :param speed_1: linear speed at timestamp 1
    :param speed_2: linear speed at timestamp 2
    :param t_1: timestamp 1
    :param t_2: timestamp 2
    ...
    if (t_2 - t_1) <= 0:
        raise TrajectoryException("bad timestamps: " + str(t_1) + " & " +
                                  str(t_2))
    return (speed_2 - speed_1) / (t_2 - t_1)

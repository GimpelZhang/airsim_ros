...
def find_the_nearest(est_position,traj_ref):
    """
    找到traj_ref中与est_position差距最小的点，返回此时的差距
    :param est_position: 待评价的点的三维坐标
    :param traj_ref: 参考曲线

    """
    # 生成一条由est_position组成的，与traj_ref等长度的曲线（ndarray）
    temp_traj = np.tile(est_position,(len(traj_ref.positions_xyz),1))
    # 两条曲线做差并求出各点的差距：
    Errors = temp_traj-traj_ref.positions_xyz
    dist_errors = [np.linalg.norm(E_i) for E_i in Errors]
    # 返回最小的差距：
    return dist_errors[np.argmin(dist_errors)]
...
class APE(PE):
    ...
    def process_data(self, data):
        ...
        if self.pose_relation == PoseRelation.translation_part:
            ...
            # self.error = [np.linalg.norm(E_i * 100) for E_i in self.E]
    ...
    def process_data_nave(self, data):
            """
            Calculates the APE on a batch of SE(3) poses from trajectories.
            新增函数，用来对比往复测试中的导航精度
            :param data: tuple (traj_ref, traj_est) with:
            ...
            if self.pose_relation == PoseRelation.translation_part:
                ...
                # self.error = [np.linalg.norm(E_i * 100) for E_i in self.E]
                i = 0
                for est_position in traj_est.positions_xyz:
                    self.error.append(find_the_nearest(est_position,traj_ref))
                    # print(i)
                    # i+=1
            else:
                raise MetricsException("unsupported pose_relation")
